...
import { JavaReader } from "./assistant";
...
export class JavaLLMReaderProvider implements vscode.WebviewViewProvider {
  ...
  async init() {
    await javaReaderAssitant?.doGC();
    javaReaderAssitant = new JavaReader(
      this.ask,
      this.say,
      this.sendError,
      this.sendState,
      ...
      // jdtls
      ((await this.getGlobalState("jdtlsPath")) as string) ??
        "/opt/homebrew/bin/jdtls",
      ((await this.getGlobalState("javaProjectPath")) as string) ?? "",
  ...
  async ask(content: string): Promise<AskResponse> {
    javaReaderAssitant?.clearWebViewAskResponse();
    ...
    await pWaitFor(
      () => {
        return !!javaReaderAssitant?.getWebViewAskResponse();
      },
      { interval: 500 }
    );
    const response: AskResponse = {
      ask: javaReaderAssitant?.getWebViewAskResponse() ?? "unknown error",
    };
    ...
    webview.onDidReceiveMessage(async (message) => {
      ...
      switch (type) {
        case "Init":
          ...
          javaReaderAssitant?.runFirstTask(
            rootPath,
            rootFunctionName,
            purpose
          );
        ...
        case "InitHistory":
          try {
            ...
            javaReaderAssitant?.runFirstTaskWithHistory(
              rootPath,
              rootFunctionName,
              purpose,
              fixedHistoryJSON
        ...
        case "Ask":
          ...
          javaReaderAssitant?.handleWebViewAskResponse(askResponse);
          break;
        case "Jdtls":
          const jdtlsPath = message.text;
          this.updateGlobalState("jdtlsPath", jdtlsPath);
          this.init();
          break;
        case "JavaProjectPath":
          const javaProjectPath = message.text;
          this.updateGlobalState("javaProjectPath", javaProjectPath);
          this.init();
          break;
        case "ReportPath":
