...
type SettingViewType = {
  ...
  initJdtlsPath: string;
  initJavaProjectPath: string;
...
const SettingView: React.FC<SettingViewType> = ({
  ...
  // jdtls
  const [jdtlsPath, setJdtlsPath] = useState<string>("");
  const updateJdtlsPath = () => {
    vscode.postMessage({
      type: "Jdtls",
      text: jdtlsPath,
    });
  ...
  // linux path
  const [javaProjectPath, setJavaProjectPath] = useState<string>("");
  const updateJavaProjectPath = () => {
    vscode.postMessage({
      type: "JavaProjectPath",
      text: javaProjectPath,
    });
  };
  // report path
  ...
  useEffect(() => {
    setJdtlsPath(initJdtlsPath);
  }, [initJdtlsPath]);
  useEffect(() => {
    setJavaProjectPath(initJavaProjectPath);
  }, [initJavaProjectPath]);
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      />
      <br />
      <VscodeButton onClick={updateJdtlsPath}>Save Jdtls Path</VscodeButton>
      <hr />
      <p>Java プロジェクトのパス</p>
      <VscodeTextfield
        value={javaProjectPath}
        onChange={(e) =>
          setJavaProjectPath(
            (e?.target as HTMLTextAreaElement)?.value ?? "error occurs"
          )
      ...
      />
      <br />
      <VscodeButton onClick={updateJavaProjectPath}>Save Java Project Path</VscodeButton>
      <hr />
      <p>レポートを保存する先のパス</p>
      <VscodeTextfield
        value={reportPath}
